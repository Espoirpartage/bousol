-- Appliquer les droits communs des rôles de direction et de trésorerie.
create or replace function public.is_finance_staff() returns boolean
language sql stable security definer set search_path = '' as $$
  select coalesce(public.current_role() in ('tresoriere','rh','direction','vice_president','tresoriere_adjointe'), false);
$$;

create or replace function public.can_manage_funds() returns boolean
language sql stable security definer set search_path = '' as $$
  select coalesce(public.current_role() in ('tresoriere','direction','vice_president','tresoriere_adjointe'), false);
$$;
...
begin
  ...
  if p_type='pret' then
    ...
    if v_role not in ('membre','tresoriere','direction','vice_president','tresoriere_adjointe') then raise exception 'Accès refusé pour accorder un prêt.'; end if;
    v_status := case when v_role in ('tresoriere','direction','vice_president','tresoriere_adjointe') then 'confirme'::public.operation_status else 'en_attente'::public.operation_status end;
...
begin
  ...
  select jsonb_build_object(
    'fund_available',case when v_role in ('tresoriere','rh','direction','vice_president','tresoriere_adjointe') then coalesce((select sum(l.amount_gdes) from public.ledger l where l.status='confirme' and l.type in ('cotisation','don','autre')),0) else 0 end,
    'monthly_contributions',case when v_role in ('tresoriere','rh','direction','vice_president','tresoriere_adjointe') then coalesce((select sum(l.amount_gdes) from public.ledger l where l.status='confirme' and l.type='cotisation' and l.created_at>=date_trunc('month',now())),0) else 0 end,
    'loans_outstanding',case when v_role in ('tresoriere','rh','direction','vice_president','tresoriere_adjointe') then greatest(coalesce((select sum(case when x.type='pret' then x.amount_gdes else -x.amount_gdes end) from public.ledger x where x.status='confirme' and x.type in ('pret','remboursement')),0),0) else 0 end,
    'active_members',case when v_role in ('tresoriere','rh','direction','vice_president','tresoriere_adjointe') then (select count(*) from public.profiles where role='membre' and active) else 0 end,
    'pending_operations',(select count(*) from public.ledger where status='en_attente' and (v_role in ('tresoriere','direction','vice_president','tresoriere_adjointe') or created_by=v_user)),
