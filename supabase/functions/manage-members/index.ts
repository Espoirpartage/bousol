...
Deno.serve(async (req) => {
  ...
  if (!profile?.active || !['tresoriere', 'direction'].includes(profile.role)) return reply({ error: 'Accès réservé à la trésorière et à la direction.' }, 403);
  try {
    ...
    if (body.action === 'invite') {
      ...
      if (!/^\S+@\S+\.\S+$/.test(email) || !fullName || fullName.length > 120 || !['membre','rh','direction','tresoriere'].includes(role)) return reply({ error: 'Nom, courriel et rôle valides requis.' }, 400);
